                            ...
                            <dl class="layui-nav-child">
                                @foreach($menu['children'] as $key2 => $item)
                                    @if(call_user_func($item['permission']))
                                        <dd _class="layui-this" id="nav_{{$key1}}_{{$key2}}">
                                            <a href="@if(!empty($item['link'])){{$item['link']}}@elseif(!empty($item['route'])){{route($item['route'], $item['params'])}}@if(!empty($item['query']))?{{implode('&',$item['query'])}}@endif @else javascript:;@endif">{{ $item['text'] }}</a>
                                        </dd>
                                    @endif
                                @endforeach
                            </dl>
